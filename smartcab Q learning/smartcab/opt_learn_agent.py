...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = 1.0/((1.0/tolerance - 1)*self.n_trial*self.n_trial/ttl_trials/ttl_trials + 1)
            #self.epsilon = pow(2.71828,  log(tolerance, 2.71828) * self.n_trial/ttl_trials)

            self.epsilon = cos(1.57 * self.n_trial/ttl_trials)
    ...
    def build_state(self):
        ...
        distance = self.planner.get_distance()
        diff = deadline - distance # used only to generate the last result in the project
        ###########
        ## TO DO ##
        ###########
        ...
        state = (waypoint, inputs['light'], inputs['oncoming'], inputs['left'], diff//5 if diff >= 0 else -1)
    ...
    def learn(self, new_state):
        ...
        if self.previous_sar:
            ...
            new_value = previous_reward + 0.8*self.get_maxQ(new_state)[0]
...
def run():
    ...
    sim.run(n_test=30, tolerance=tolerance)
